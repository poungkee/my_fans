...
const rssSources_1 = require("../config/rssSources");
class RSSCrawlerService {
    ...
    async extractContentFromURL(url, sourceName) {
        try {
    ...
    // 한겨레 전용 추출 함수
    async extractFromHani(html, url) {
    ...
    // 경향신문 전용 추출 함수
    async extractFromKhan(html, url) {
        const $ = cheerio.load(html);
    ...
    async extractFromDonga(html, url) {
        const $ = cheerio.load(html);
        let content = '';
    ...
    // 조선일보 전용 추출 함수
    async extractFromChosun(html, url) {
        const $ = cheerio.load(html);
    ...
    // 범용 추출 함수 (기타 언론사용)
    async extractFromGeneric(html, url, sourceName) {
        const $ = cheerio.load(html);
        // 범용 선택자들
        const selectors = [
    ...
    cleanArticleContent(content) {
        ...
        if (lastLine && lastLine.length < 20 && (lastLine.includes('기자') ||
    ...
    extractValidJournalistNames(rawName) {
        if (!rawName)
            return '';
    ...
    extractActualSourceFromURL(url) {
        try {
            const urlObj = new URL(url);
    ...
    async crawlAllRSSFeeds(limitPerFeed = 4) {
        ...
        const results = {};
        for (const feed of rssSources_1.RSS_FEEDS) {
            try {
    ...
    // 지원하는 RSS 피드 목록 반환
    getSupportedFeeds() {
        return rssSources_1.RSS_FEEDS.map(feed => feed.sourceName);
    }
}
