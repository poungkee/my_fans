...
exports.analyzeBias = analyzeBias;
...
async function analyzeBias() {
    ...
    `, [BATCH_SIZE]);
        ...
        if (articles.length === 0) {
            logger_1.logger.info('⚖️  편향 분석할 정치 기사가 없습니다');
            return;
        }
        logger_1.logger.info(`⚖️  ${articles.length}개 정치 기사 편향 분석 중...`);
        let successCount = 0;
        let failCount = 0;
        // 2. 각 기사에 대해 편향 분석
        for (const article of articles) {
            try {
                // Bias Analysis AI 호출
                const response = await axios_1.default.post(`${BIAS_ANALYSIS_AI_URL}/analyze/political`, {
                    text: `${article.title}\n\n${article.content}`,
                ...
                if (response.data) {
                    ...
                    await client.query(`INSERT INTO bias_analysis (
              article_id,
              political_leaning,
              bias_score,
              confidence,
            ...
            DO UPDATE SET
              ...
              analyzed_at = NOW()`, [
                    ...
                    successCount++;
                    logger_1.logger.debug(`✅ 기사 ID ${article.id} 편향 분석 완료: ${biasData.stance}`);
                }
                else {
                ...
                failCount++;
                logger_1.logger.error(`❌ 기사 ID ${article.id} 편향 분석 실패: ${error.message}`);
                continue;
            }
            // API 호출 제한을 위한 짧은 지연
            await new Promise(resolve => setTimeout(resolve, 150));
        }
        logger_1.logger.info(`✅ 편향 분석 완료: ${successCount}개 성공, ${failCount}개 실패`);
    }
    catch (error) {
        logger_1.logger.error(`❌ 편향 분석 작업 실패: ${error.message}`);
        throw error;
    }
    finally {
        client.release();
