/**
 * 키워드 추출 작업
 * Queue 기반으로 변경됨
 */

import { logger } from '../utils/logger';
import { getDbClient } from '../utils/database';
import { addBatchKeywordJobs } from '@fans/queue';

const BATCH_SIZE = parseInt(process.env.KEYWORD_BATCH_SIZE || '50');

interface Article {
  id: number;
  title: string;
  content: string;
}

/**
 * 키워드 추출 메인 함수
 */
export async function extractKeywords(): Promise<void> {
  const client = await getDbClient();

  try {
    // 1. 키워드가 없는 기사 조회
    const result = await client.query(`
      SELECT id, title, content
      FROM news_articles
      WHERE id NOT IN (SELECT DISTINCT news_id FROM news_keywords)
        AND content IS NOT NULL
        AND LENGTH(content) >= 100
      ORDER BY created_at DESC
      LIMIT $1
    `, [BATCH_SIZE]);

    const articles: Article[] = result.rows;

    if (articles.length === 0) {
      logger.info('🔑 키워드 추출할 기사가 없습니다');
      return;
    }

    logger.info(`🔑 ${articles.length}개 기사를 키워드 추출 큐에 추가 중...`);

    // 2. 모든 기사를 Queue에 배치 추가
    try {
      const jobs = articles.map(article => ({
        articleId: article.id,
        content: article.content,
        title: article.title
      }));

      const jobIds = await addBatchKeywordJobs(jobs);
      logger.info(`✅ 키워드 추출 큐에 ${jobIds.length}개 작업 추가 완료`);
    } catch (error: any) {
      logger.error(`❌ 키워드 추출 큐 추가 실패: ${error.message}`);
      throw error;
    }
  } catch (error: any) {
    logger.error(`❌ 키워드 추출 작업 실패: ${error.message}`);
    throw error;
  } finally {
    client.release();
  }
}
